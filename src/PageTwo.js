...
function PageTwo() {
   ...
   return (<div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
         ...
         {errors?.address && (<div className="error-address errors">{errors.address.message}</div>)}
         ...
         {errors.rentPrice && errors.rentPrice.type === "required" && (<div className="errors error-rentPrice error-rent-aligh-right">это поле нужно заполнить</div>)}

         {errors.rentPrice && errors.rentPrice.type === "min" && (<div className="errors error-rentPrice">Мы работаем с квартирами от 20 000₽/мес.<br />
         <span>Напишите на <span style={{textDecoration: 'underline'}}>help@pik-arenda.ru</span> и мы найдем решение</span></div>)}
         
         {errors.rentPrice && errors.rentPrice.type === "max" && (<div className="errors error-rentPrice">Мы работаем с квартирами до 100 000₽/мес.<br />
         <span>Напишите на <span style={{textDecoration: 'underline'}}>help@pik-arenda.ru</span> и мы найдем решение</span></div>)}
         ...
         {errors?.fileDoc && (<div className="errors error-file-doc">{errors.fileDoc.message}</div>)}
      ...
      <button onClick={() => navigate(-1)} className="btn-back"><span className="text-btn">Назад</span></button>
   </div>)
}
